...
class Dialogo_crear_apuesta(QDialog):
    ...
    def __init__(self, apostadores, competidores, apuesta=None):
        ...
        if apuesta != None:
            ...
            '''PASO2: Conservar los datos actuales'''
            #self.valor_actual=str(self.texto_valor.text())
            self.apostador_actual=str(self.combobox_apostadores.currentText())
            self.competidor_actual=str(self.combobox_competidores.currentText())
    ...
    def guardar(self):
        ...
        self.valor_nuevo=str(self.texto_valor.text())

        '''PASO3: Conservar los datos nuevos y enviarlos como return'''
        if(self.valor_nuevo!=""):
            self.apostador_nuevo=str(self.combobox_apostadores.currentText())
            self.competidor_nuevo=str(self.combobox_competidores.currentText())

            self.resultado=1
            self.close()
            return self.resultado, self.apostador_actual, self.competidor_actual, self.valor_nuevo,self.apostador_nuevo,self.competidor_nuevo
