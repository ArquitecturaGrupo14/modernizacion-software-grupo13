...
class Vista_carrera(QWidget):
    ...
    def aniadir_competidor(self):
        ...
        if dialogo.resultado == 1:
            self.competidores.append({'nombre':dialogo.texto_nombre.text(), 'probabilidad':float(dialogo.texto_probabilidad.text()), 'Estado':'Nueva','Carrera':self.texto_nombre.text()})
            self.mostrar_competidores(self.texto_nombre.text(), self.competidores)
    ...
    def guardar_cambios(self):
        ...
        """    
        cantidad_competidores=len(self.competidores)
        if(cantidad_competidores>1):
            if(self.texto_nombre.text()!=""):
                suma_probabilidad=0
                for j, pro_competidor in enumerate(self.competidores):
                    suma_probabilidad+=pro_competidor['probabilidad']
                if(suma_probabilidad==1):    
                    ...
                    for i, competidor in enumerate(self.competidores):
                        if competidor.get('Estado') == 'Nueva':
                            self.interfaz.aniadir_competidor(competidor['nombre'], competidor['probabilidad'],competidor['Carrera'])
                    ...
                    self.hide()
                    self.interfaz.mostrar_vista_lista_carreras()
                else: QMessageBox.information(self, 'Mensaje', "La suma de las probabilidades debe ser igual a 1")
            else:
                QMessageBox.information(self, 'Mensaje', "Por favor Ingresa nombre de la carrera")
        else:
                QMessageBox.information(self, 'Mensaje', "La cantidad de competidores debe ser mayor a 1")